...
from ptan.ignite import EpisodeFPSHandler, EndOfEpisodeHandler, EpisodeEvents, PeriodicEvents, PeriodEvents
from torch import device
from torch.optim import Adam

from Chapter8.lib import common, dqn_model
from Chapter9.lib import common

BATCH_MUL = 4
EpisodeEnded = namedtuple("EpisodeEnded", field_names="reward steps epsilon")
NAME = "03_parallel"
...
class BatchGenerator:
    def __init__(self, buffer: ExperienceReplayBuffer, exp_queue: mp.Queue, fps_handler: EpisodeFPSHandler,
...
if __name__ == "__main__":
    ...
    buffer = ExperienceReplayBuffer(experience_source=None, buffer_size=params.replay_size)
    ...
    def process_batch(engine, batch):
        ...
        if engine.state.iteration % params.target_net_sync == 0:
            tgt_net.sync()
        return {"loss": loss.item(), "epsilon": batch_generator.epsilon}


    engine = Engine(process_batch)
    EndOfEpisodeHandler(batch_generator, bound_avg_reward=17.0).attach(engine)
    ...
    logdir = f"runs/{now}-{params.run_name}-{NAME}"
